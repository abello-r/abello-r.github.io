...
function ft_get_data_user(e)
{
	...
	fetch(`https://api.intra.42.fr/v2/users/${user}`,{
		...
		.then(async (datos)=>
		{
			...
			power = await datos.correction_point + datos.wallet + datos.achievements.length + (datos.projects_users.length * 0.5);
...
function ft_get_data_user_two(e)
{
	...
	fetch(`https://api.intra.42.fr/v2/users/${user}`,{
		...
		.then(async (datos)=>
		{
			...
			power_two = await datos.correction_point + datos.wallet + datos.achievements.length + (datos.projects_users.length * 0.5);
...
function ft_win_phrases()
{
	...
	else if (pick.selectedOptions[0].value == "Iron-Man") // IronMan man phrs
	{
