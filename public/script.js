...
pick_two.addEventListener("change", () => {
	pick_two_img.setAttribute(
		"src",
		"public/src/" + pick_two.selectedOptions[0].value + ".ico"
	)
})
...
pick_one.addEventListener("change", () => {
	pick_one_img.setAttribute(
		"src",
		"public/src/" + pick_one.selectedOptions[0].value + ".ico"
	)
})
...
function ft_get_data_user(e) {
	...
	fetch(`https://api.intra.42.fr/v2/users/${user}`, {
		...
		.then((response) => response.json())
		.then(async (datos) => {
			if (datos.id == null || datos.id == undefined)
				return console.error("No data") // TODO: Handle error...
			console.log(datos)
...
function ft_victory(e) {
	if (power > 0 && power_two > 0) {
		if (power > power_two) {
			...
			document
				.getElementById("crown_one")
				.setAttribute("src", "public/src/" + "crown.ico") // Set crown for the king
			...
			document
				.getElementById("crown_two")
				.setAttribute("src", "public/src/draw.ico") // Draw ico
			document
				.getElementById("crown_one")
				.setAttribute("src", "public/src/draw.ico") // Draw ico
			document.getElementById("victory").innerHTML =
